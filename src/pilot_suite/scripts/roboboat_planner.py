...
from pilot_suite.msg import RawWaypoint, WaypointReached, SkipWaypoint
...
Sends these raw waypoints to the pilot_suite/local_planner node, which processes them and then sends them to
pilot_suite/waypoint_pilot
...
class WaypointGenerator():
    def __init__(self):
        ...
        self.waypoint_pub = rospy.Publisher('pilot_suite/raw_waypoint', RawWaypoint, queue_size=10)
        self.reached_waypoint_sub = rospy.Subscriber('pilot_suite/waypoint_reached', WaypointReached, self.reached_waypoint_callback)
        self.waypoint_skip_pub = rospy.Publisher('pilot_suite/waypoint_skip', SkipWaypoint)
    ...
    def send_waypoint(self, position, direction, waypoint_id, is_dir_vector=True, ignore_local_planner=False):
        """
        Send a waypoint to the pilot_suite node with the given position and direction.
