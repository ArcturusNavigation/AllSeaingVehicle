...
class SkeeballTaskNode(TaskNode):
    ...
    def __init__(self, detection_method='combined', bag_bounds=(250, 300), flip_point=(360, 640)):
        ...
        self.center_pub = rospy.Publisher(
            '/pilot_suite/skeeball_task/target_center_pose', Point, queue_size=1)
        self.marker_pub = rospy.Publisher(
            '/pilot_suite/skeeball_task/debug/marker_pub', Marker, queue_size=1)

        if(self.debug):
            self.image_segmented_pub = rospy.Publisher(
                '/pilot_suite/skeeball_task/debug/segmented_img', Image, queue_size=1)
            self.image_filtered_pub = rospy.Publisher(
                '/pilot_suite/skeeball_task/debug/filtered_img', Image, queue_size=1)
            self.image_outliers_pub = rospy.Publisher(
                '/pilot_suite/skeeball_task/debug/outliers_img', Image, queue_size=1)
    ...
    def identify_center(self, input_img):
        ...
        def remove_outliers(x_blues, y_blues):
            ...
            variances =  np.square(y_blues - y_mean)

            mean_var = np.mean(variances)
            if mean_var == 0:
                return x_blues, y_blues

            x_blues, y_blues = x_blues[variances / mean_var <= self.VAR_THRESHOLD1], y_blues[variances / mean_var <= self.VAR_THRESHOLD1]
            ...
            return x_blues[variances / mean_var <= self.VAR_THRESHOLD2], y_blues[variances / mean_var <= self.VAR_THRESHOLD2]
