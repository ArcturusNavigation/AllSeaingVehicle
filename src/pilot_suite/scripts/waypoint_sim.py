...
import numpy as np
import skimage
from AllSeaingVehicle.src.pilot_suite.scripts.utils.geom_helper import quaternion_from_angle
import pygame
import random

from geometry_msgs.msg import PoseStamped, Point
from nav_msgs.msg import OccupancyGrid
from pilot_suite.msg import ProcessedWaypoint, WaypointReached, SkipWaypoint
...
class WaypointSim():
    def __init__(self):
        ...
        self.boat_pos = np.array(
            convert_image_coords(40, 220), dtype=np.float32)
        ...
        self.waypoint_sub = rospy.Subscriber(
            'pilot_suite/processed_waypoint', ProcessedWaypoint, self.waypoint_callback)
        self.pose_pub = rospy.Publisher(
            'pilot_suite/pose', PoseStamped, queue_size=10)
        self.objects_pub = rospy.Publisher(
            'sensor_suite/objects', ObjectArray, queue_size=5)
        self.occupancy_pub = rospy.Publisher(
            '/occupancy_grid', OccupancyGrid, queue_size=5)
        self.waypoint_reached_pub = rospy.Publisher(
            'pilot_suite/waypoint_reached', WaypointReached, queue_size=10)
        rospy.Subscriber('pilot_suite/waypoint_skip',
                         SkipWaypoint, self.waypoint_skip_callback)
    ...
    def draw(self):
        ...
        pygame.draw.circle(self.screen, (0, 0, 255), get_screen_coords(
            self.boat_pos[0], self.boat_pos[1]), 5)

        waypoint, status = self.get_curr_waypoint()
        # rospy.logerr(str(waypoint))
        if waypoint != None:
            pygame.draw.circle(self.screen, (255, 0, 255) if status else (
                0, 0, 0), get_screen_coords(waypoint[0], waypoint[1]), 5)
        ...
        for obstacle in self.other_obstacles:
            converted_obstacle = [get_screen_coords(
                x, y) for (x, y) in obstacle]
    ...
    def get_curr_waypoint(self):
        while self.current_order in self.skipped_waypoints:
            ...
            self.waypoint_reached_pub.publish(
                WaypointReached(self.current_order))
    ...
    def step(self):
        ...
        if waypoint is None:
            ...
            rospy.loginfo("setting setpoint: " + str(waypoint_x) +
                          " " + str(waypoint_y) + " " + str(waypoint_heading))
            ...
            if abs(heading_step) > STEP_SIZE_HEADING:
                heading_step = STEP_SIZE_HEADING * \
                    heading_step / abs(heading_step)
            self.boat_heading += heading_step

            dist_sq = (self.boat_pos[0] - waypoint_x) ** 2 + \
                (self.boat_pos[1] - waypoint_y) ** 2
