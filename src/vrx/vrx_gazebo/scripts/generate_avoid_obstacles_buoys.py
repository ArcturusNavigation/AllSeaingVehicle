...
def get_models_xml(arr, model):
    ...
    for idx, pos in enumerate(arr):
        ret += TEMPLATE.format(
            pos=(pos[0], pos[1], 0.0), model=model, name=model + "_" + str(idx)
        )
    return ret
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Generates an SDF model for the RobotX 2018 obstacle course challenge."
    )
    parser.add_argument(
        "-s",
        "--seed",
    ...
    parser.add_argument(
        "--a3", type=int, default=5, help="Number of a3 buoys to include in challenge"
    )
    parser.add_argument(
        "--a5", type=int, default=5, help="Number of a5 buoys to include in challenge"
    )
    parser.add_argument(
        "--a7", type=int, default=5, help="Number of a7 buoys to include in challenge"
    )
    parser.add_argument(
        "--surmark46104",
        type=int,
    ...
    parser.add_argument(
        "--N", type=int, default=0, help="Generates N markers of each type"
    )
    parser.add_argument(
        "--L", type=float, default=40, help="Size of box used for generation (LxL)"
    )
    ...
    arr = generate_obstacle_course(
        np.array([args.L, args.L]),
        total,
        5.0,
        ul=np.array([-args.L / 2.0, -args.L / 2.0]),
    )
    ...
    if args.a5 > 0:
        ...
        a5_xml = ""
    if args.a7 > 0:
        a7_xml = get_models_xml(
            arr[args.a3 + args.a5 : args.a3 + args.a5 + args.a7, :], "polyform_a7"
        )
    else:
        a7_xml = ""
    if args.surmark46104 > 0:
        surmark46104_xml = get_models_xml(
            arr[args.a3 + args.a5 + args.a7 : total, :], "surmark46104"
        )
    else:
        surmark46104_xml = ""
