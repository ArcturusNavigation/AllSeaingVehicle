...
class Node:
    ...
    def callback(self, data):
        ...
        elif (self.input_msg_type == "Pose") or (self.input_msg_type == "Imu"):
            pass
        elif self.input_msg_type == "Odometry":
            data = data.pose.pose
        else:
            rospy.logerr(
                "Don't know what to do with message type %s" % self.input_msg_type
            )
            sys.exit()
        q = (
            data.orientation.x,
        ...
        rospy.logdebug(
            "publishing rpy: %.2f, %.2f, %.2f" % (euler[0], euler[1], euler[2])
        )
...
if __name__ == "__main__":
    ...
    if not (model_name == None):
        ...
        if inmsgtype == "Pose":
            ...
            rospy.logerr("I don't know how to deal with message type <%s>" % inmsgtype)
            sys.exit()

    rospy.loginfo("Subscribing to %s, looking for %s messages." % (in_topic, inmsgtype))

    rospy.loginfo("Publishing to %s, sending Vector3 messages" % (out_topic))
