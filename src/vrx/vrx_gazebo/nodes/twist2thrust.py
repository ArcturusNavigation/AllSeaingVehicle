...
class Node:
    ...
    def callback(self, data):
        ...
        rospy.logdebug(
            "\t\tx:%f,y:%f,z:%f" % (data.linear.x, data.linear.y, data.linear.z)
        )
        rospy.logdebug("\tangular:")
        rospy.logdebug(
            "\t\tx:%f,y:%f,z:%f" % (data.angular.x, data.angular.y, data.angular.z)
        )
        # scaling factors
        ...
        rospy.logdebug("TX ")
        rospy.logdebug(
            "\tleft:%f, right:%f" % (self.left_msg.data, self.right_msg.data)
        )
...
if __name__ == "__main__":
    ...
    node.shooter_pub = rospy.Publisher(
        "/wamv/shooters/ball_shooter/fire", Empty, queue_size=1
    )
