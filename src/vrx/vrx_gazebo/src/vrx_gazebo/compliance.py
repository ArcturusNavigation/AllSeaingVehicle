...
class ComponentCompliance:
    def __init__(self):
        ...
        self.boxes = find_boxes(
            os.path.join(self.config_dir, "component_compliance", "bounding_boxes.yaml")
        )
        ...
        self.numeric = yaml.safe_load(
            open(os.path.join(self.config_dir, "component_compliance", "numeric.yaml"))
        )
        return
    ...
    def param_compliance(self, component_type, params={}):
        ...
        assert (
            component_type in self.default_parameters
        ...
        if "x" and "y" and "z" in params:
            ...
            for box in self.boxes:
                if box.fit(xyz):
                    return True
            rospy.logerr("%s %s is out of bounds" % (component_type, params["name"]))
            rospy.logerr(
...
class ThrusterCompliance:
    def __init__(self):
        ...
        self.boxes = find_boxes(
            os.path.join(self.config_dir, "thruster_compliance", "bounding_boxes.yaml")
        )
        ...
        self.numeric = yaml.safe_load(
            open(os.path.join(self.config_dir, "thruster_compliance", "numeric.yaml"))
        )
        return
    ...
    def param_compliance(self, thruster_type, params={}):
        ...
        if thruster_type not in self.default_parameters:
            rospy.logerr(
                "%s is not defined anywhere under %s" % (thruster_type, self.config_dir)
            )
        assert (
            thruster_type in self.default_parameters
        ), "%s is not defined anywhere under %s" % (thruster_type, self.config_dir)
        for i in params:
            if i not in self.numeric[thruster_type]["allowed_params"]:
                rospy.logerr(
                    "%s parameter specification of not permitted" % (i, thruster_type)
                )
                assert False
        ...
        xyz = np.array(
            [float(j) for j in [i for i in params["position"].split(" ") if i != ""]]
        )
        for box in self.boxes:
            if box.fit(xyz):
                return True
...
class Box:
    def __init__(self, name, pose, size, space):
        ...
        self.pose = np.array(
            [float(j) for j in [i for i in pose.split(" ") if i != ""]]
        )
        self.size = np.array(
            [float(j) for j in [i for i in size.split(" ") if i != ""]]
        )
