...
def main():
    ...
    create_urdf_command = (
        "rosrun xacro xacro -o " + wamv_target + " '" + wamv_gazebo + "'"
    )

    if wamv_locked:
        create_urdf_command += " locked:=" + str(wamv_locked)
    ...
    if received_thruster_yaml:
        ...
        create_urdf_command += (
            ...
            "thruster_xacro_file:=" + thruster_xacro_target
        )
    if received_component_yaml:
...
def create_thruster_xacro():
    ...
    compliant = create_xacro_file(
        yaml_file=thruster_yaml,
...
def create_component_xacro():
    ...
    return create_xacro_file(
        yaml_file=component_yaml,
