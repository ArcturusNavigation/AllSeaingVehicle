...
GUITaskWidget::GUITaskWidget()
  ...
  //// cppcheck-suppress 3;
  windPixmap(150, 150)
  , windPainter(&(this->windPixmap))
  ,
  // setup pixmap and painter for contact
  contactPixmap(150, 150)
  , contactPainter(&(this->contactPixmap))
  ,
  ...
  contactTime(ros::Time::now())
{
  ...
  connect(this, SIGNAL(SetTaskInfo(QString)), taskInfo, SLOT(setText(QString)), Qt::QueuedConnection);
  ...
  connect(this, SIGNAL(SetWindDirection(QPixmap)), windDirection, SLOT(setPixmap(QPixmap)), Qt::QueuedConnection);
  ...
  connect(this, SIGNAL(SetContact(QPixmap)), contact, SLOT(setPixmap(QPixmap)), Qt::QueuedConnection);
  ...
  this->taskSub = this->node->subscribe("/vrx/task/info", 1, &GUITaskWidget::OnTaskInfo, this);
  // Subscribe to wind speed topic (ROS)
  this->windSpeedSub = this->node->subscribe("/vrx/debug/wind/speed", 1, &GUITaskWidget::OnWindSpeed, this);
  // Subscribe to wind direction topic (ROS)
  this->windDirectionSub = this->node->subscribe("/vrx/debug/wind/direction", 1, &GUITaskWidget::OnWindDirection, this);
  // Subscribe to link states topic (ROS)
  this->linkStateSub = this->node->subscribe("/gazebo/link_states", 1, &GUITaskWidget::OnLinkStates, this);
  // Subscribe to contact topic (ROS)
  this->contactSub = this->node->subscribe("/vrx/debug/contact", 1, &GUITaskWidget::OnContact, this);
#endif
}
...
void GUITaskWidget::OnLinkStates(const gazebo_msgs::LinkStates::ConstPtr& _msg)
{
  ...
  tf::Quaternion q(_msg->pose[c].orientation.x, _msg->pose[c].orientation.y, _msg->pose[c].orientation.z,
