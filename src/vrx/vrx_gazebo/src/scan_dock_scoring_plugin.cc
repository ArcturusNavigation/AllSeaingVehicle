...
ColorSequenceChecker::ColorSequenceChecker(const std::vector<std::string>& _expectedColors,
  ...
  : expectedSequence(_expectedColors), ns(_rosNameSpace), colorSequenceService(_rosColorSequenceService)
{
...
{
  this->colorSequenceServer =
      this->nh.advertiseService(this->colorSequenceService, &ColorSequenceChecker::OnColorSequence, this);
}
...
bool ColorSequenceChecker::OnColorSequence(
    ros::ServiceEvent<vrx_gazebo::ColorSequence::Request, vrx_gazebo::ColorSequence::Response>& _event)
{
  ...
  this->correctSequence =
      color1 == this->expectedSequence[0] && color2 == this->expectedSequence[1] && color3 == this->expectedSequence[2];
  if (this->correctSequence)
  {
    ROS_INFO_NAMED("ColorSequenceChecker",
                   "Received color sequence is "
...
/////////////////////////////////////////////////
DockChecker::DockChecker(const std::string& _name, const std::string& _internalActivationTopic,
                         const std::string& _externalActivationTopic, const double _minDockTime,
                         const bool _dockAllowed, const std::string& _worldName, const std::string& _rosNameSpace,
                         const std::string& _announceSymbol, const std::string& _gzSymbolTopic)
  : name(_name)
  ...
  , gzSymbolTopic(_gzSymbolTopic)
{
  ...
  this->ignNode.Subscribe(this->internalActivationTopic, &DockChecker::OnInternalActivationEvent, this);
  this->ignNode.Subscribe(this->externalActivationTopic, &DockChecker::OnExternalActivationEvent, this);
#else
  this->containSub =
      this->node->Subscribe(this->internalActivationTopic, &DockChecker::OnInternalActivationEvent, this);
  this->containSub =
      this->node->Subscribe(this->externalActivationTopic, &DockChecker::OnExternalActivationEvent, this);
#endif
}
...
void DockChecker::AnnounceSymbol()
{
  ...
  if (this->dockAllowed)
  {
    ...
    this->symbolPub = this->nh->advertise<std_msgs::String>(this->symbolTopic, 1, true);
...
void DockChecker::Update()
{
  ...
  this->anytimeDocked = this->timer.GetElapsed() >= gazebo::common::Time(this->minDockTime);

  if (this->anytimeDocked)
  {
    gzmsg << "Successfully stayed in dock for " << this->minDockTime << " seconds, transitioning to <docked> state"
          << std::endl;
  }
}
  ...
  if (_msg.data() == 0)
  {
    ...
    if (this->AnytimeDocked())
    {
      gzmsg << "Leaving internal dock activation zone in [" << this->name
            << "] after required time - transitioning to <exited> state." << std::endl;
    }
    else
    {
      gzmsg << "Leaving internal dock activation zone in [" << this->name
            << "] early - transitioning back to <undocked> state." << std::endl;
    }
  }

  gzdbg << "[" << this->name << "] OnInternalActivationEvent(): " << _msg.data() << std::endl;
}
...
void DockChecker::OnExternalActivationEvent(const ignition::msgs::Boolean& _msg)
{
  ...
  if (this->atEntrance)
  {
    gzmsg << "Entering external dock activation zone in [" << this->name << "]" << std::endl;
  }
  else
  {
    gzmsg << "Leaving external dock activation zone in [" << this->name << "]" << std::endl;
  }

  gzdbg << "[" << this->name << "] OnExternalActivationEvent(): " << _msg.data() << std::endl;
}
#else
/////////////////////////////////////////////////
...
//////////////////////////////////////////////////
ScanDockScoringPlugin::ScanDockScoringPlugin() : node(new gazebo::transport::Node())
{
}

/////////////////////////////////////////////////
void ScanDockScoringPlugin::Load(gazebo::physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
  this->node->Init();
  ...
  this->updateConnection =
      gazebo::event::Events::ConnectWorldUpdateBegin(std::bind(&ScanDockScoringPlugin::Update, this));
...
bool ScanDockScoringPlugin::ParseSDF(sdf::ElementPtr _sdf)
{
  ...
  {
    enableColorChecker = _sdf->GetElement("enable_color_checker")->Get<bool>();
  }
  ...
  {
    colorSequenceService = _sdf->GetElement("color_sequence_service")->Get<std::string>();
  }
  ...
  {
    this->colorBonusPoints = _sdf->GetElement("color_bonus_points")->Get<double>();
  }

  // Instantiate the color checker.
  if (this->enableColorChecker)
  {
    this->colorChecker.reset(new ColorSequenceChecker(this->expectedSequence, ns, colorSequenceService));
  }
  ...
  while (bayElem)
  {
    ...
    std::string internalActivationTopic = bayElem->Get<std::string>("internal_activation_topic");
    ...
    std::string externalActivationTopic = bayElem->Get<std::string>("external_activation_topic");
...
// Create a new dock checker.
#if GAZEBO_MAJOR_VERSION >= 8
    std::unique_ptr<DockChecker> dockChecker(new DockChecker(bayName, internalActivationTopic, externalActivationTopic,
                                                             minDockTime, dockAllowed, this->world->Name(), ns,
                                                             announceSymbol, symbolTopic));
#else
    std::unique_ptr<DockChecker> dockChecker(new DockChecker(bayName, internalActivationTopic, externalActivationTopic,
                                                             minDockTime, dockAllowed, this->world->GetName(), ns,
                                                             announceSymbol, symbolTopic));
#endif
  ...
  {
    this->dockBonusPoints = _sdf->GetElement("dock_bonus_points")->Get<double>();
  }

  // Optional: the points granted when the vehicle docks in the expected bay.
  if (_sdf->HasElement("correct_dock_bonus_points"))
  {
    this->correctDockBonusPoints = _sdf->GetElement("correct_dock_bonus_points")->Get<double>();
  }
...
#if GAZEBO_MAJOR_VERSION >= 8
      std::function<void(const ignition::msgs::Boolean&)> subCb = [this,
                                                                   bonusPoints](const ignition::msgs::Boolean& _msg) {
...
void ScanDockScoringPlugin::Update()
{
  if (this->enableColorChecker)
  {
    ...
    if (!this->colorSubmissionProcessed && this->colorChecker->SubmissionReceived())
    {
