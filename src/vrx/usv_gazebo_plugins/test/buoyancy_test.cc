...
TEST(PolyhedronTest, CubeNotSubmerged)
{
  ...
  auto volume = cube.SubmergedVolume(ignition::math::Vector3d{ 0, 0, 2 }, ignition::math::Quaterniond{ 1, 0, 0, 0 },
                                     waterSurface);
...
TEST(PolyhedronTest, CylinderNotSubmerged)
{
  ...
  auto volume = cylinder.SubmergedVolume(ignition::math::Vector3d{ 0, 0, 2 }, ignition::math::Quaterniond{ 1, 0, 0, 0 },
                                         waterSurface);
...
TEST(PolyhedronTest, CubeSubmerged)
{
  ...
  // -------|   |--------
  //        -----
  auto volume = cube.SubmergedVolume(ignition::math::Vector3d{ 0, 0, 0 }, ignition::math::Quaterniond{ 1, 0, 0, 0 },
                                     waterSurface);
  ...
  //        |   |
  //        -----
  volume = cube.SubmergedVolume(ignition::math::Vector3d{ 0, 0, -2 }, ignition::math::Quaterniond{ 1, 0, 0, 0 },
                                waterSurface);
  ...
  volume = cube.SubmergedVolume(ignition::math::Vector3d{ 0, 0, 0.25 },
                                ignition::math::Quaterniond{ 0.9238795, 0.3826834, 0, 0 }, waterSurface);
...
TEST(PolyhedronTest, CylinderSubmerged)
{
  ...
  auto volume = cylinder.SubmergedVolume(ignition::math::Vector3d{ 0, 0, 0.0 },
                                         ignition::math::Quaterniond{ 1, 0, 0, 0 }, waterSurface);
  ...
  //        | |
  //        ---
  volume = cylinder.SubmergedVolume(ignition::math::Vector3d{ 0, 0, -4 }, ignition::math::Quaterniond{ 1, 0, 0, 0 },
                                    waterSurface);
  EXPECT_NEAR(volume.volume, 1.57, 0.01);
  EXPECT_FLOAT_EQ(volume.centroid.X(), 0.0);
  EXPECT_FLOAT_EQ(volume.centroid.Y(), 0.0);
  ...
  volume = cylinder.SubmergedVolume(ignition::math::Vector3d{ 0, 0, 0 },
                                    ignition::math::Quaterniond{ 0.707, 0.707, 0, 0 }, waterSurface);
  EXPECT_NEAR(volume.volume, 0.785, 0.001);
  EXPECT_FLOAT_EQ(volume.centroid.X(), 0.0);
  EXPECT_FLOAT_EQ(volume.centroid.Y(), 0.0);
...
TEST(ShapeVolumeTest, CubeNoRotation)
{
  ...
  const std::vector<std::vector<double>> poses = { { 0, 0, 1.0 },  { 0, 0, 0.5 }, { 0, 0, 0.0 },
                                                   { 0, 0, -0.5 }, { 0, 0, -1. }, { 0, 0, -1.5 } };
  const std::vector<std::vector<double>> expectedResult = { { 0, 0, 0, 0 },     { 2, 0, 0, -0.25 }, { 4, 0, 0, -0.5 },
                                                            { 6, 0, 0, -0.75 }, { 8, 0, 0, -1.0 },  { 8, 0, 0, -1.5 } };
...
TEST(ShapeVolumeTest, CubeRotation)
{
  ...
  ShapeVolumePtr box = std::move(ShapeVolume::makeShape(boxElem));
  const double fluidLevel = 0;
  const std::vector<std::vector<double>> poses = { { 0, 0, 1.5 },  { 0, 0, 1.0 }, { 0, 0, 0.5 }, { 0, 0, 0.0 },
                                                   { 0, 0, -0.5 }, { 0, 0, -1. }, { 0, 0, -1.5 } };
  const std::vector<std::vector<double>> expectedResult = { { 0, 0, 0, 0 },          { 0.343, 0, 0, -0.138 },
                                                            { 1.672, 0, 0, -0.305 }, { 4.000, 0, 0, -0.471 },
                                                            { 6.328, 0, 0, -0.713 }, { 7.657, 0, 0, -1.051 },
                                                            { 8.000, 0, 0, -1.500 } };
  ignition::math::Pose3d pose;
...
TEST(ShapeVolumeTest, CylinderNoRotation)
{
  ...
  const std::vector<std::vector<double>> expectedResult = { { 0, 0, 0, 0 },       { 0.39, 0, 0, -0.25 },
                                                            { 0.77, 0, 0, -0.5 }, { 1.16, 0, 0, -0.75 },
                                                            { 1.55, 0, 0, -1.0 }, { 1.55, 0, 0, -1.5 } };
  ignition::math::Pose3d pose;
  for (size_t i = 0; i < poses.size(); i++)
  {
    pose.Pos().X() = poses[i][0];
...
TEST(ShapeVolumeTest, CylinderRotation)
{
  ...
  ShapeVolumePtr cylinder = std::move(ShapeVolume::makeShape(cylinderElem));
  const double fluidLevel = 0;
  const std::vector<std::vector<double>> poses = { { 0, 0, 0.5 }, { 0, 0, 0.0 }, { 0, 0, -0.5 } };
  // note since we are approximating final volume is 1.55 instead of 1.57
  // (increase segments for precision)
  const std::vector<std::vector<double>> expectedResult = {
    { 0, 0, 0, 0 }, { 0.77, 0, 0, -0.21 }, { 1.55, 0, 0, -0.5 }, { 1.55, 0, 0, -1.0 }
  };
  ignition::math::Pose3d pose;
...
TEST(ShapeVolumeTest, Sphere)
{
  ...
  const double fluidLevel = 0;
  const std::vector<std::vector<double>> poses = { { 0, 0, 0.5 },   { 0, 0, 0.25 }, { 0, 0, 0.0 },
                                                   { 0, 0, -0.25 }, { 0, 0, -0.5 }, { 0, 0, -1.0 } };

  const std::vector<std::vector<double>> expectedResult = { { 0, 0, 0, 0 },          { 0.082, 0, 0, -0.088 },
                                                            { 0.262, 0, 0, -0.188 }, { 0.442, 0, 0, -0.313 },
                                                            { 0.523, 0, 0, -0.500 }, { 0.523, 0, 0, -1.000 } };
