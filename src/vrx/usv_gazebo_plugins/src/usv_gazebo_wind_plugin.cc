...
void UsvWindPlugin::Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  {
    gzwarn << "VRX_DEBUG environment variable not set, defaulting to true" << std::endl;
  }
  this->world = _parent;
  ...
  else
  {
    ...
    while (windObjSDF)
    {
      ...
      if (!windObjSDF->HasElement("name") || !windObjSDF->GetElement("name")->GetValue())
      {
      ...
      if (!windObjSDF->HasElement("link_name") || !windObjSDF->GetElement("link_name")->GetValue())
      {
      ...
      if (!windObjSDF->HasElement("coeff_vector") || !windObjSDF->GetElement("coeff_vector")->GetValue())
      {
  ...
  {
    this->windMeanVelocity = _sdf->GetElement("wind_mean_velocity")->Get<double>();
  }
  ...
  {
    this->gainConstant = _sdf->GetElement("var_wind_gain_constants")->Get<double>();
  }
  ...
  {
    this->timeConstant = _sdf->GetElement("var_wind_time_constants")->Get<double>();
  }
  ...
  {
    this->topicWindSpeed = _sdf->GetElement("topic_wind_speed")->Get<std::string>();
  }
  ...
  {
    this->topicWindDirection = _sdf->GetElement("topic_wind_direction")->Get<std::string>();
  }
  ...
  if (_sdf->HasElement("random_seed") && _sdf->GetElement("random_seed")->Get<unsigned int>() != 0)
  {
  ...
  this->windSpeedPub = this->rosNode->advertise<std_msgs::Float64>(this->topicWindSpeed, 100);
  this->windDirectionPub = this->rosNode->advertise<std_msgs::Float64>(this->topicWindDirection, 100);

  // Listen to the update event. This event is broadcast every
  // simulation iteration.
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(std::bind(&UsvWindPlugin::Update, this));
}
...
void UsvWindPlugin::Update()
{
  ...
  if (!this->windObjsInit)
  {
    for (auto& i : this->windObjs)
    {
      ...
      if ((!i.init) && (this->world->GetModel(i.modelName)))
      {
        ...
        if (!i.link)
        {
          gzdbg << i.modelName << "'s link name: " << i.linkName << " is invalid" << std::endl;
        }
      }
    }
  ...
  this->varVel += 1.0 / this->timeConstant * (-1.0 * this->varVel + this->filterGain / sqrt(dT) * randomDist) * dT;
  ...
  for (auto& windObj : this->windObjs)
  {
    ...
    ignition::math::Vector3d relativeWind =
        windObj.link->WorldPose().Rot().Inverse().RotateVector(this->windDirection * velocity);
#else
    ignition::math::Vector3d relativeWind =
        windObj.link->GetWorldPose().rot.Ign().Inverse().RotateVector(this->windDirection * velocity);
#endif
    // Calculate apparent wind
#if GAZEBO_MAJOR_VERSION >= 8
    ignition::math::Vector3d apparentWind = relativeWind - windObj.link->RelativeLinearVel();
#else
    ignition::math::Vector3d apparentWind = relativeWind - windObj.link->GetRelativeLinearVel().Ign();
#endif
    ...
    ignition::math::Vector3d windForce(windObj.windCoeff.X() * relativeWind.X() * abs(relativeWind.X()),
    ...
    windObj.link->AddRelativeForce(ignition::math::Vector3d(windForce.X(), windForce.Y(), 0.0));
    windObj.link->AddRelativeTorque(ignition::math::Vector3d(0.0, 0.0, windForce.Z()));
  }
  ...
  if ((currentTime - this->lastPublishTime > publishingBuffer) && this->debug)
  {
    ...
    windDirectionMsg.data = atan2(this->windDirection[1], this->windDirection[0]) * 180 / M_PI;
