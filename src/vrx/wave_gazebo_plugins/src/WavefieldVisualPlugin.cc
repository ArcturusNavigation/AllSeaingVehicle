...
void ToOgreVector2(const std::vector<ignition::math::Vector2d>& _v, Ogre::Vector2& _vout0, Ogre::Vector2& _vout1,
                   Ogre::Vector2& _vout2)
{
...
void ToOgreVector3(const std::vector<ignition::math::Vector3d>& _v, Ogre::Vector3& _vout0, Ogre::Vector3& _vout1,
                   Ogre::Vector3& _vout2)
{
...
{
public:
  WavefieldVisualPluginPrivate() : planeUp("planeUp"), planeDown("planeDown")
  {
  }
...
WavefieldVisualPlugin::WavefieldVisualPlugin()
  : VisualPlugin(), RenderTargetListener(), data(new WavefieldVisualPluginPrivate)
{
  this->data->isInitialised = false;
}

void WavefieldVisualPlugin::Load(rendering::VisualPtr _visual, sdf::ElementPtr _sdf)
{
  // @DEBUG_INFO
...
// Process SDF Parameters
  ...
  gzmsg << "WavefieldVisualPlugin <" << this->data->visualName << ">: Loading WaveParamaters from SDF" << std::endl;

  this->data->isStatic = Utilities::SdfParamBool(*_sdf, "static", false);

#if (GAZEBO_MAJOR_VERSION == 7 && GAZEBO_MINOR_VERSION >= 16) ||                                                       \
    (GAZEBO_MAJOR_VERSION == 9 && GAZEBO_MINOR_VERSION >= 11) || (GAZEBO_MAJOR_VERSION > 9)
  ...
  this->data->refractOpacity = Utilities::SdfParamDouble(*_sdf, "refractOpacity", 0.2);
  this->data->reflectOpacity = Utilities::SdfParamDouble(*_sdf, "reflectOpacity", 0.2);
  this->data->rttNoise = Utilities::SdfParamDouble(*_sdf, "rttNoise", 0.1);
  ...
  this->data->oceanEntity = dynamic_cast<Ogre::Entity*>(ogreNode->getAttachedObject(0));
  ...
  this->data->oceanEntity->setRenderQueueGroup(this->data->oceanEntity->getRenderQueueGroup() + 1);
  ...
  this->data->preRenderConnection =
      event::Events::ConnectPreRender(std::bind(&WavefieldVisualPlugin::OnPreRender, this));
}
...
#else
    rendering::SetMaterialShaderParam(*this->data->visual, "time", shaderType, std::to_string(0.0));
#endif
...
void WavefieldVisualPlugin::OnPreRender()
{
  ...
  if (!this->data->isStatic)
  {
    ...
    rendering::SetMaterialShaderParam(*this->data->visual, "time", "vertex",
...
void WavefieldVisualPlugin::SetupReflectionRefraction()
{
  ...
  this->data->planeUp = Ogre::MovablePlane(Ogre::Vector3::UNIT_Z, Ogre::Vector3::ZERO);
  this->data->planeDown = Ogre::MovablePlane(-Ogre::Vector3::UNIT_Z, Ogre::Vector3::ZERO);

  // Get texture unit states to update with rtts
  Ogre::MaterialPtr material = Ogre::MaterialManager::getSingleton().getByName(this->data->visual->GetMaterialName());
  this->data->reflectTex = (material->getTechnique(0)->getPass(0)->getTextureUnitState(2));

  this->data->refractTex = (material->getTechnique(0)->getPass(0)->getTextureUnitState(3));

  // Set reflection/refraction parameters
  rendering::SetMaterialShaderParam(*this->data->visual, "refractOpacity", "fragment",
                                    std::to_string(static_cast<float>(this->data->refractOpacity)));
  rendering::SetMaterialShaderParam(*this->data->visual, "reflectOpacity", "fragment",
                                    std::to_string(static_cast<float>(this->data->reflectOpacity)));
  rendering::SetMaterialShaderParam(*this->data->visual, "rttNoise", "fragment",
  ...
  {
    rendering::SetMaterialShaderParam(*this->data->visual, "flipAcrossY", "fragment", std::to_string(0));
  }
  else
  {
    rendering::SetMaterialShaderParam(*this->data->visual, "flipAcrossY", "fragment", std::to_string(1));
  }
  ...
  this->data->cameraPreRenderConnection = rendering::Events::ConnectCameraPreRender(
      std::bind(&WavefieldVisualPlugin::OnCameraPreRender, this, std::placeholders::_1));
#endif
}
...
void WavefieldVisualPlugin::UpdateClipPlanes()
{
  ...
  Ogre::Vector3 oceanPosition(pose.Pos().X(), pose.Pos().Y(), pose.Pos().Z());

  Ogre::Quaternion oceanRotation(pose.Rot().W(), pose.Rot().X(), pose.Rot().Y(), pose.Rot().Z());
#else
...
void WavefieldVisualPlugin::CreateRtts(Ogre::Camera* _camera)
{
  ...
  Ogre::TexturePtr rttReflectionTexture = Ogre::TextureManager::getSingleton().createManual(
      this->data->visualName + "_" + _camera->getName() + "_reflection",
      Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, kWidth, kHeight, 0, Ogre::PF_R8G8B8,
      Ogre::TU_RENDERTARGET);

  // Create refraction texture
  Ogre::TexturePtr rttRefractionTexture = Ogre::TextureManager::getSingleton().createManual(
      this->data->visualName + "_" + _camera->getName() + "_refraction",
      Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, kWidth, kHeight, 0, Ogre::PF_R8G8B8,
      Ogre::TU_RENDERTARGET);

  Ogre::ColourValue backgroundColor = rendering::Conversions::Convert(this->data->scene->BackgroundColor());

  // Setup reflection render target
  Ogre::RenderTarget* reflectionRt = rttReflectionTexture->getBuffer()->getRenderTarget();
  reflectionRt->setAutoUpdated(false);
  Ogre::Viewport* reflVp = reflectionRt->addViewport(_camera);
  reflVp->setClearEveryFrame(true);
  reflVp->setOverlaysEnabled(false);
  reflVp->setBackgroundColour(backgroundColor);
  reflVp->setVisibilityMask(GZ_VISIBILITY_ALL & ~(GZ_VISIBILITY_GUI | GZ_VISIBILITY_SELECTABLE));
  reflectionRt->addListener(this);

  // Setup refraction render target
  Ogre::RenderTarget* refractionRt = rttRefractionTexture->getBuffer()->getRenderTarget();
  refractionRt->setAutoUpdated(false);
  Ogre::Viewport* refrVp = refractionRt->addViewport(_camera);
  refrVp->setClearEveryFrame(true);
  refrVp->setOverlaysEnabled(false);
  refrVp->setBackgroundColour(backgroundColor);
  refrVp->setVisibilityMask(GZ_VISIBILITY_ALL & ~(GZ_VISIBILITY_GUI | GZ_VISIBILITY_SELECTABLE));
...
std::vector<rendering::CameraPtr> WavefieldVisualPlugin::NewCameras()
{
  ...
  for (auto sensor : sensors)
  {
    ...
    {
      rendering::CameraPtr c = this->data->scene->GetCamera(this->data->scene->StripSceneName(sensor->ScopedName()));

      if (c && std::find(this->data->cameras.begin(), this->data->cameras.end(), c->OgreCamera()) ==
                   this->data->cameras.end())
      {
        retVal.push_back(c);
      }
...
#else
  rendering::SetMaterialShaderParam(visual, "Nwaves", shaderType, std::to_string(this->data->waveParams->Number()));
  rendering::SetMaterialShaderParam(visual, "amplitude", shaderType, Ogre::StringConverter::toString(amplitude));
  rendering::SetMaterialShaderParam(visual, "wavenumber", shaderType, Ogre::StringConverter::toString(wavenumber));
  rendering::SetMaterialShaderParam(visual, "omega", shaderType, Ogre::StringConverter::toString(omega));
  rendering::SetMaterialShaderParam(visual, "steepness", shaderType, Ogre::StringConverter::toString(steepness));
  rendering::SetMaterialShaderParam(visual, "dir0", shaderType, Ogre::StringConverter::toString(dir0));
  rendering::SetMaterialShaderParam(visual, "dir1", shaderType, Ogre::StringConverter::toString(dir1));
  rendering::SetMaterialShaderParam(visual, "dir2", shaderType, Ogre::StringConverter::toString(dir2));
  float tau = this->data->waveParams->Tau();
  rendering::SetMaterialShaderParam(visual, "tau", shaderType, Ogre::StringConverter::toString(tau));
#endif
}
