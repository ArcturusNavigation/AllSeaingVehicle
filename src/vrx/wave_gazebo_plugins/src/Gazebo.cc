                            ...
                            const std::string& _value)
{
  ...
  {
    gzerr << "Shader type: '" << _shaderType << "' is not supported" << std::endl;
    return;
  }

  // set the parameter based name and type defined in material script
  // and shaders
  auto setNamedParam = [](Ogre::GpuProgramParametersSharedPtr _params, const std::string& _name,
                          const std::string& _v) {
  ...
  Ogre::MaterialPtr mat = Ogre::MaterialManager::getSingleton().getByName(_visual.GetMaterialName());
  if (mat.isNull())
  {
    gzerr << "Failed to find material: '" << _visual.GetMaterialName() << std::endl;
    return;
  }
  for (unsigned int i = 0; i < mat->getNumTechniques(); ++i)
  {
    ...
    for (unsigned int j = 0; j < technique->getNumPasses(); ++j)
    {
      ...
      {
        gzerr << "Failed to retrieve shaders for material: '" << _visual.GetMaterialName() << "', technique: '"
              << technique->getName() << "', pass: '" << pass->getName() << "'" << std::endl;
        continue;
      }
    }
  }
