...
public:
  void RecalculateCmr()
  {
    ...
    for (size_t i = 0; i < this->number; ++i)
    {
      ...
      const ignition::math::Vector2d d(c * this->direction.X() - s * this->direction.Y(),
...
public:
  double pm(double omega, double omega_p)
  {
    double alpha = 0.0081;
    double g = 9.81;
    return alpha * std::pow(g, 2.0) / std::pow(omega, 5.0) * std::exp(-(5.0 / 4.0) * std::pow(omega_p / omega, 4.0));
  }
...
public:
  void Recalculate()
  {
    ...
    {
      gzmsg << "Using Pierson-Moskowitz spectrum sampling wavefield model " << std::endl;
      this->RecalculatePms();
    }
    else if (!this->model.compare("CWR"))
    {
      gzmsg << "Using Constant wavelength-ampltude ratio wavefield model " << std::endl;
      this->RecalculateCmr();
    }
    else
    {
...
WaveParameters::WaveParameters() : data(new WaveParametersPrivate())
{
...
double WavefieldSampler::ComputeDepthSimply(const WaveParameters& _waveParams, const ignition::math::Vector3d& _point,
...
double WavefieldSampler::ComputeDepthDirectly(const WaveParameters& _waveParams, const ignition::math::Vector3d& _point,
                                              double time, double time_init)
{
  // Struture for passing wave parameters to lambdas
  struct WaveParams
  {
    WaveParams(const std::vector<double>& _a, const std::vector<double>& _k, const std::vector<double>& _omega,
               const std::vector<double>& _phi, const std::vector<double>& _q,
  ...
  WaveParams wp(_waveParams.Amplitude_V(), _waveParams.Wavenumber_V(), _waveParams.AngularFrequency_V(),
                _waveParams.Phase_V(), _waveParams.Steepness_V(), _waveParams.Direction_V());
