...
WavefieldModelPlugin::WavefieldModelPlugin() : ModelPlugin(), data(new WavefieldModelPluginPrivate())
{
}
...
void WavefieldModelPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  this->data->updateConnection =
      event::Events::ConnectWorldUpdateBegin(std::bind(&WavefieldModelPlugin::OnUpdate, this));
  ...
  this->data->wavefieldEntity->SetName(WavefieldEntity::MakeName(this->data->model->GetName()));
...
std::shared_ptr<const WaveParameters> WavefieldModelPlugin::GetWaveParams(gazebo::physics::WorldPtr _world,
