...
class WaterGunController:
	def __init__(self):
		...
		ts = message_filters.ApproximateTimeSynchronizer(
		    [self.image_sub, self.info_sub], 10, .5, allow_headerless=True)
	...
	def callback(self, image_msg, info_msg):
		...
		if self.camera is None:
			...
			self.camera.fromCameraInfo(info_msg)
		try:
		...
		if center_2d is not None: 
			center_3d = self.camera.projectPixelTo3dRay(center_2d)
	...
	def find_center(self, img):
		...
		if len(bounding_boxes) > 0:
			...
			print("No blue quadrants detected :(")
		return None,frame
